...
interface TimeTrackerOriginalLayoutProps {
...
export function TimeTrackerOriginalLayout({
  isWorkDayOver,
  isValid,
  completionTime,
  currentTime,
  ...
  onAddManualBreak,
  workDoneMs,
}: TimeTrackerOriginalLayoutProps) {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="flex flex-col gap-6 h-full font-sans">
        {/* Hero Timer Card */}
        <Card className={`text-center shadow-lg flex-none transition-all duration-300 hover:shadow-xl ${isWorkDayOver ? "border-orange-500/50 shadow-orange-500/10" : "border-primary/20 shadow-primary/5"}`}>
          ...
          <CardContent className="space-y-4 pb-4 pt-2">
            <div className="flex flex-col items-center justify-center space-y-2">
              <div className={`font-mono tabular-nums tracking-tighter font-extrabold text-5xl sm:text-6xl ${isWorkDayOver ? "text-orange-600 drop-shadow-sm" : "text-primary drop-shadow-sm"}`}>
              ...
              {isValid && (
                <div className="flex items-center gap-2 text-muted-foreground bg-muted/40 px-3 py-1 rounded-full animate-in fade-in slide-in-from-bottom-2 mt-0.5">
                  ...
                  <span className="text-lg font-mono font-bold text-foreground">{formatTime(workDoneMs)}</span>
                </div>
              )}
              {isWorkDayOver && (
            ...
            <div className="space-y-2.5">
              ...
              <Progress value={isValid ? progress : 0} className={`h-3 w-full rounded-full ${isWorkDayOver ? "bg-orange-100 dark:bg-orange-950/30" : "bg-primary/10"}`} />
            </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-muted/40 border border-muted/60 transition-colors hover:bg-muted/60">
                  ...
                  <div className="flex items-center gap-1.5 text-foreground">
                    ...
                    <span className="text-xl font-mono font-bold tracking-tight">{format(completionTime, "hh:mm a")}</span>
                  </div>
                </div>
                <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-muted/40 border border-muted/60 transition-colors hover:bg-muted/60">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground mb-1">Current Time</span>
                  <div className="flex items-center gap-1.5 text-foreground">
                    <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                    <span className="text-xl font-mono font-bold tracking-tight">{format(currentTime, "hh:mm a")}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-none">
          ...
          <Card className="shadow-lg flex flex-col hover:shadow-xl transition-all duration-300 border-green-500/20 bg-gradient-to-br from-card to-green-500/5">
            <CardHeader className="pb-2">
              <CardTitle className="font-headline text-lg flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" onClick={startEditingDuration}>
            ...
            <CardContent className="flex flex-col flex-grow justify-center items-center py-4 relative">
              <div className={`flex flex-col items-center space-y-6 w-full transition-opacity duration-200 ${isEditingDuration ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <div className="text-center group relative cursor-pointer" onClick={() => !isEditingStartTime && setIsEditingStartTime(true)}>
                  ...
                  {isEditingStartTime ? (
                    <div className="flex items-center gap-2 animate-in fade-in zoom-in-95">
                      <Input type="time" value={arrivalTime ? format(arrivalTime, "HH:mm") : ""} onChange={handleTimeInputChange} className="w-auto font-mono text-lg" autoFocus onBlur={handleSaveStartTime} onKeyDown={(e) => e.key === 'Enter' && handleSaveStartTime()} />
                      <Button onClick={handleSaveStartTime} size="sm" className="h-10">Save</Button>
                    </div>
                  ) : (
                ...
                <div className="flex items-center justify-center gap-3 w-full">
                  <Button variant={activeDurationMode === "full" ? "default" : "secondary"} size="sm" className="flex-1 transition-all active:scale-95 shadow-sm" onClick={() => onSetWorkDuration("full")}>Full Day</Button>
                  <Button variant={activeDurationMode === "half" ? "default" : "secondary"} size="sm" className="flex-1 transition-all active:scale-95 shadow-sm" onClick={() => onSetWorkDuration("half")}>Half Day</Button>
                </div>
              </div>
              {isEditingDuration && (
                  ...
                  <div className="w-full bg-muted/30 p-4 rounded-lg border border-muted space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        <Input value={tempHours} onChange={(e) => setTempHours(e.target.value)} className="h-9 font-mono text-center" placeholder="Hrs" />
                        <span className="text-[10px] text-muted-foreground text-center block mt-1">Hours</span>
                      </div>
                      <span className="font-bold">:</span>
                      <div className="flex-1">
                        <Input value={tempMinutes} onChange={(e) => setTempMinutes(e.target.value)} className="h-9 font-mono text-center" placeholder="Min" />
          ...
          {/* Break Tracker Card */}
          <Card className={`shadow-lg flex flex-col hover:shadow-xl transition-all duration-300 ${isOnBreak ? "border-amber-500 bg-amber-500/5" : "border-amber-500/20"}`}>
            <CardHeader className="pb-2">
              <CardTitle className="font-headline text-lg flex items-center justify-between">
                <span className="flex items-center gap-2">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-foreground [&_svg]:size-5" onClick={() => setIsAddingBreak(!isAddingBreak)}>
                      <Icons.Pencil />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent><p>Manage Break</p></TooltipContent>
                </Tooltip>
              </CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col flex-grow justify-center items-center py-6 relative">
              <div className={`flex flex-col items-center w-full transition-opacity duration-300 ${isAddingBreak ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <div className={`font-mono tabular-nums tracking-tighter font-bold text-5xl sm:text-6xl mb-4 ${isOnBreak ? "text-amber-600 animate-pulse" : "text-foreground"}`}>
                  {formatTime(totalBreakMs)}
                </div>
                <Button variant={isOnBreak ? "destructive" : "secondary"} className={`w-full max-w-[200px] font-semibold transition-all shadow-sm ${isOnBreak ? "hover:bg-red-600" : "hover:bg-secondary/80"}`} onClick={onToggleBreak} size="lg">
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-card/95 backdrop-blur-sm transition-all animate-in fade-in duration-200 rounded-lg">
                  <div className="w-full space-y-3">
                    ...
                    <div className="flex items-center gap-2 justify-center">
                      <div className="flex-1">
                        <Input value={manualBreakMinutes} onChange={(e) => setManualBreakMinutes(e.target.value)} className="h-9 font-mono text-center" placeholder="Min" type="number" autoFocus onKeyDown={(e) => e.key === 'Enter' && handleAddBreak("add")} />
                  ...
                  {logs.slice().reverse().map((log, index) => (
                    <div key={index} className="flex items-center justify-between text-sm group hover:bg-muted/50 p-2 rounded-md transition-colors">
                      <div className="flex items-center gap-3">
                        <div className={`p-1.5 rounded-full ${log.type === 'punch-in' ? 'bg-green-100 dark:bg-green-900/30 text-green-600' : log.type === 'break-start' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : log.type === 'break-end' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <span className="font-mono text-xs text-muted-foreground/70">{format(new Date(log.timestamp), "hh:mm:ss a")}</span>
                    </div>
                  ))}
                </div>
              </ScrollArea>
