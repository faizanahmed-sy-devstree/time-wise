    ...
    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 lg:h-[calc(100vh-250px)]">
      ...
      <div className="lg:col-span-8 flex flex-col gap-4 lg:h-full order-1">
        ...
        <Card className="relative p-6 lg:p-10 rounded-[2rem] lg:rounded-[2.5rem] bg-white dark:bg-[#09090b] border-zinc-200 dark:border-white/5 shadow-2xl dark:shadow-none overflow-hidden flex flex-col justify-center min-h-[350px] lg:flex-1">
          {/* Ambient Glow - Toggled for theme */}
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            {/* 4-Column Grid Stats (Now with Light Theme support) */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 w-full">
              ...
              <StatBlock 
                ...
                icon={<Icons.Target className="h-4 w-4 text-rose-600 dark:text-rose-400" />}
              />
            </div>
            ...
            <div className="w-full mt-8 space-y-2">
               <div className="flex justify-between items-center px-1">
                  <span className="text-[10px] font-semibold text-zinc-400 dark:text-white/30 tracking-tight">Shift progress</span>
                  <span className="text-xs font-black text-primary">{Math.round(stats.progress)}%</span>
               </div>
               <div className="h-2 w-full bg-zinc-100 dark:bg-white/5 rounded-full overflow-hidden p-0.5 border border-zinc-200 dark:border-white/5">
                  <div 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 order-3 lg:order-2">
          ...
          <FooterMetric label="Exceptions" value={`${monthStats?.gracePeriod.lateIn}In • ${monthStats?.gracePeriod.earlyOut}Out`} />
        </div>
      </div>
      ...
      <Card className="lg:col-span-4 p-5 lg:p-6 rounded-[2rem] lg:rounded-[2.5rem] bg-white dark:bg-card border-zinc-200 dark:border-white/5 shadow-xl dark:shadow-none flex flex-col lg:h-full order-2 lg:order-3 overflow-hidden">
        <div className="flex items-center justify-between mb-6 flex-none">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm lg:text-md font-bold tracking-tight text-zinc-900 dark:text-white">Activity history</h3>
          </div>
        ...
        <ScrollArea className="flex-1 min-h-0 pr-4">
          <div className="relative pl-8 space-y-6 lg:space-y-8 py-2">
            {/* Timeline Line: Light/Dark adaptive */}
                ...
                <div key={i} className="relative flex items-center justify-between group">
                  ...
                  <div className={cn(
                    ...
                    <Icons.RefreshCcw className={cn(
                      ...
                      isPunchIn ? "text-emerald-600 dark:text-emerald-500" : "text-orange-600 dark:text-orange-500 rotate-180"
                    )} />
                  </div>
                </div>
              );
...
function StatBlock({ label, value, icon, className }: { label: string; value: string; icon: React.ReactNode; className: string }) {
  return (
    <div className={cn("p-3 lg:p-4 border rounded-[1.2rem] lg:rounded-[1.5rem] flex flex-col gap-1 transition-all shadow-sm dark:shadow-lg group", className)}>
